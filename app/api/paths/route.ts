...
async function generatePath(answers: Answers): Promise<LearningPath> {
  ...
  if (typeof content !== "string") throw new Error("El modelo devolvió una respuesta vacía. Vuelve a intentar.");
  let result: unknown;
  try {
  ...
  if (typeof data.title !== "string" || !data.title.trim() || data.title.length > 100 || typeof data.description !== "string" || !data.description.trim() || data.description.length > 800 || typeof data.advice !== "string" || !data.advice.trim() || data.advice.length > 600 || !Array.isArray(data.recommendedSlugs) || !data.recommendedSlugs.every((slug: unknown) => typeof slug === "string" && courses.some((course) => course.slug === slug && course.priority === "recomendado"))) {
    throw new Error("La ruta generada no cumple el formato del catálogo. Vuelve a intentar.");
  }
  const recommended = data.recommendedSlugs as string[];
  return {
...
const getCachedPath = unstable_cache(generatePath, ["devpathlles-v1", JSON.stringify(programs)], { revalidate: 86400 });
